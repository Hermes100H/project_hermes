...
def CSVsaver(optim_result_profile, circuit: Circuit):
    ...
    try:
        ...
        pass
    with open(f'./optim_results/test/circuitt_{date}_coeffs_{circuit.coeffs[0]}_{circuit.coeffs[1]}_{circuit.coeffs[2]}_start_{circuit.start_x}_end_{circuit.end_x}_len_{circuit.segment_length}.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['profile', 'timing', 'energy'])
        columns = [tuple(optim_result_profile), tuple(calcTimings(optim_result_profile, circuit)), tuple(EnergieDepenseParInstantSpatial(optim_result_profile, circuit))]
        rows = zip(*columns)
...
def print_optim_info(boost_profile_optimal: OptimizeResult, circuit: Circuit, optim_method: str):
    ...
    print(f'Energie totale du profil optimal : {sum(EnergieDepenseParInstantSpatial(boost_profile_optimal.x, circuit))}')
...
def CSVsaver(optim_result_profile, circui: Circuit):
    ...
    try:
        ...
        pass
    with open(
        f"../optim_results/test/circuit_{date}_coeffs_{circui.coeffs[0]}_{circui.coeffs[1]}_{circui.coeffs[2]}_start_{circui.start_x}_end_{circui.end_x}_len_{circui.segment_length}.csv",
        "w",
        newline="",
    ) as file:
        ...
        columns = [
            ...
            tuple(calcTimings(optim_result_profile, circui)),
            tuple(EnergieDepenseParInstantSpatial(optim_result_profile, circui)),
        ]
        rows = zip(*columns)
